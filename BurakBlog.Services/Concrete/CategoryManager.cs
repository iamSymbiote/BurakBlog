...
using BurakBlog.Shared.Utilities.Results.Concrete;
using System.Collections;

namespace BurakBlog.Services.Concrete
{
    public class CategoryManager : ICategoryService
    {
        ...
        public async Task<IDataResult<CategoryDto>> Add(CategoryAddDto categoryAddDto, string createdByName)
        {
            ...
            // AutoMapper kullanmazsaniz kodunuz buna benzeyebilir << https://i.hizliresim.com/bvdwrqw.png >> 
            ...
            //    IsDeleted = false
            //}).ContinueWith(t => _unitOfWork.SaveAsync()); //alttaki saveAsync gibi islemin database'e yazmasini beklemeden frontend'e cakip gosterecek `continuewith`
            //await _unitOfWork.SaveAsync();
        ...
        {
            var categories = await _unitOfWork.Categories.GetAllAsync(c => !c.IsDeleted, c => c.Articles); // ` !c.IsDeleted ` c.IsDeleted == false yaziminin baska hali.
            if (categories.Count > -1)
            {
                return new DataResult<CategoryListDto>(ResultStatus.Success, new CategoryListDto
                {
        ...
        {
            var categories = await _unitOfWork.Categories.GetAllAsync(c => !c.IsDeleted && c.IsActive, c => c.Articles); // ` !c.IsDeleted ` c.IsDeleted == false yaziminin baska hali.
            if (categories.Count > -1)
            {
                return new DataResult<CategoryListDto>(ResultStatus.Success, new CategoryListDto
                {
